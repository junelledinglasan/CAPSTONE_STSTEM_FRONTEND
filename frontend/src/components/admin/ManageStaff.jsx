...
function AddStaffModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = () => {
    ...
    const result = addStaff({ name: form.name, username: form.username, password: form.password });
    if (!result.success) { setErrors({ username: result.message }); return; }
    setDone(result.staff);
    onSuccess?.();
  };
    ...
    <div className="ms-overlay" onClick={onClose}>
      <div className="ms-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ms-modal-footer">
          ...
          <button className="ms-btn-save" onClick={handleSubmit}>Create Staff Account</button>
        </div>
      </div>
    </div>
  );
...
function EditStaffModal({ staff, onClose, onSuccess }) {
  ...
  const handleSubmit = () => {
    ...
    const result = editStaff(staff.id, { name: form.name, username: form.username });
    if (!result.success) { setErrors({ username: result.message }); return; }
    setDone(true);
    onSuccess?.();
  };
  ...
  return (
    <div className="ms-overlay" onClick={onClose}>
      <div className="ms-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ms-modal-footer">
          <button className="ms-btn-cancel" onClick={onClose}>Cancel</button>
          <button className="ms-btn-save" onClick={handleSubmit}>Save Changes</button>
        </div>
      </div>
    </div>
  );
...
function ResetPasswordModal({ staff, onClose }) {
  ...
  const handleSubmit = () => {
    ...
    if (form.password !== form.confirm) e.confirm = "Passwords do not match.";
    if (Object.keys(e).length) { setErrors(e); return; }
    resetStaffPassword(staff.id, form.password);
    setDone(true);
  };
    ...
    <div className="ms-overlay" onClick={onClose}>
      <div className="ms-modal ms-modal-sm" onClick={e => e.stopPropagation()}>
        <div className="ms-modal-body" style={{ alignItems:"center", textAlign:"center", padding:"32px 24px", gap:12 }}>
          ...
          <div style={{ fontSize:12, color:"#555", lineHeight:1.6 }}>
            New password for <strong>{staff.name}</strong> has been set. Give them the new password.
          </div>
          <div className="ms-cred-box">
      ...
      <div className="ms-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ms-modal-body">
          <div className="ms-staff-strip">
            <div className="ms-staff-avatar">{staff.initials}</div>
            <div>
        ...
        <div className="ms-modal-footer">
          <button className="ms-btn-cancel" onClick={onClose}>Cancel</button>
          <button className="ms-btn-save" onClick={handleSubmit}>Reset Password</button>
        </div>
      </div>
    </div>
  );
...
export default function ManageStaff() {
  const { getStaffList } = useAuth();

  const [showAdd,    setShowAdd]    = useState(false);
  const [editTarget, setEditTarget] = useState(null);
  const [resetTarget,setResetTarget]= useState(null);
  const [refresh,    setRefresh]    = useState(0); // trigger re-render after changes
  ...
  return (
    <div className="ms-wrap">
      ...
      {/* Modals */}
      {showAdd     && <AddStaffModal    onClose={() => setShowAdd(false)}    onSuccess={forceRefresh} />}
      {editTarget  && <EditStaffModal   staff={editTarget} onClose={() => setEditTarget(null)}  onSuccess={forceRefresh} />}
      ...
      <div className="ms-header">
        ...
        <button className="ms-add-btn" onClick={() => setShowAdd(true)}>
          + Add Staff
        </button>
      </div>
                ...
                <tr key={s.id}>
                  ...
                  <td>
                    <div className="ms-name-cell">
                      <div className="ms-avatar">{s.initials}</div>
                      <span>{s.name}</span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="ms-action-row">
                      <button className="ms-action-btn edit" onClick={() => setEditTarget(s)}>
                        ✏️ Edit
                      </button>
                      <button className="ms-action-btn reset" onClick={() => setResetTarget(s)}>
                        🔑 Reset Password
                      </button>
                    </div>
                  </td>
